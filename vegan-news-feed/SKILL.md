...
Ajastettu ajo (LaunchAgent) ei aina onnistu — kone voi olla unessa/sammuksissa koko ajastetun hetken ympäriltä, tai (todennettu 2026-09-18, ks. `PROCESS.md`:n iteraatio 13) koneella ei ole yhtään pitkäkestoista, oikeaa heräämistä koko päivän aikana, vain toistuvia muutaman sekunnin "DarkWake"-ylläpitopiikkejä, jolloin `claude -p`:n monivaiheinen ajo ei ehdi valmistua ennen kuin kone nukahtaa uudelleen — jolloin ajo jää kokonaan tekemättä, hiljaa, ilman virheilmoitusta (keskeytetty prosessi ei osu `trap ERR`:iin). Kumpikin tapaus näkyy identtisesti: se päivä puuttuu historiasta kokonaan. Tämä haittaa erityisesti `vegan-news-feed-review`-skilliä, joka arvioi historiaa: aukko näyttää samalta kuin "ei uutisia tänään" ilman run-merkintää tarkistamatta erikseen. Tätä varten on olemassa erillinen, **pyydettäessä ajettava** kyky (ei osa päivittäistä automaatiota — ks. peruste lopussa) täyttää puuttuvat päivät jälkikäteen.
...
**Todennettu oikeasti, ei vain teoriaa (2026-09-18):** neljä aukkopäivää täytettiin samalla kertaa, kaksi 4-5 vrk vanhaa (09-14, 09-15) ja kaksi 8-14 vrk vanhaa (09-04, 09-10). Tuoreemmat kaksi täyttyivät normaalisti täydellä sisällöllä. Vanhemmille kahdelle löytyi laajalla haulla enää yksittäisiä osumia (yksi ohut julkkiskokki-featuuri, yksi jo toisena päivänä uudelleen lähetettynä duplikaattina), ei mitään todellista uutta sisältöä — molemmat merkittiin `"result": "unrecoverable"`. Tämä vahvistaa "1-5 vrk" -nyrkkisäännön suoraan mitattuna, ei vain oletettuna.

**Suomenkielisten Google News -hakujen erityispiirre backfillissä:** `references/feeds.md`:n suomenkieliset rivit sisältävät `when:2d`-määreen (ks. tiedoston "Huomioita"-osio ja `PROCESS.md`:n iteraatio 9) — tämä rajaa Googlen palauttamat tulokset AINA noin kahteen päivään NYKYHETKESTÄ, riippumatta `fetch_feeds.py`:n omasta `--hours`-arvosta. Käytännössä tämä tarkoittaa ettei `feeds.md`:n suomenkielisiä rivejä sellaisenaan voi käyttää yli parin päivän vanhojen aukkojen täyttämiseen. Vanhempia suomenkielisiä aukkoja varten rakenna tilapäiset, laajemmalla `when:Nd`-arvolla varustetut hakukyselyt (sama `hl=fi&gl=FI&ceid=FI:fi`, korvaa `when:2d` esim. `when:14d`:llä) suoraan `https://news.google.com/rss/search`-rajapintaan, älä luota `fetch_feeds.py`:n suomenkielisiin riveihin vanhemmille aukoille.
