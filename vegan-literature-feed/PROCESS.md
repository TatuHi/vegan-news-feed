...
## 6. Iteraatio 3: `DEFAULT_QUERY` tiivistettiin oikealla ennen/jälkeen-datalla

Käyttäjä pyysi 2026-09-04 tiukentamaan `fetch_pubmed.py`:n oletuskyselyä jo aiemmin kerätyn diagnostiikan (iteraatio 1:n 60 päivän testi, ~30-40 % osuvuus) pohjalta. Sen sijaan että olisi vain arvattu parempi kysely, muutos suunniteltiin ja todennettiin oikeasti PubMedin `esearch`-rajapintaa vasten ennen kuin sitä otettiin käyttöön `fetch_pubmed.py`:ssä.

**Juurisyyn tarkennus.** Molemmissa aiemmissa testeissä (iteraatio 1:n 60 päivän testi, ja tämän päivän 30 kohteen oikea ajo) kohina keskittyi selvästi kahteen paljaaseen OR-termiin: `"animal welfare"[tiab]` ja `"animal cognition"[tiab]`. Nämä ovat niin yleisiä sanoja rutiininomaisessa eläinlääketieteessä (kirurgiset tekniikat, laitteistokehitys, immunomääritykset, tuotantoeläinten sairaudet) että ne tuottivat valtaosan vääristä osumista ilman minkäänlaista eettistä/oikeuksellista kytköstä.

**Korjaus, todennettu ennen käyttöönottoa.** Muutettiin nämä kaksi termiä vapaista OR-ehdoista pakollisiksi rinnakkaisehdoiksi: `("animal welfare"[tiab] OR "animal cognition"[tiab])` vaatii nyt rinnalleen jonkin eettisyyteen/tuntoisuuteen viittaavan sanan (`policy`, `ethics`, `consumer`, `public attitudes`, `legislation`, `pain`, `sentience`, `emotion`, `consciousness`). Testattiin suoraan `esearch`-rajapinnalla 30 päivän ikkunassa ennen koodimuutosta:
- Vanha kysely: 162 osumaa.
- Ensimmäinen tiukennettu versio (vain policy/ethics/consumer/public attitudes/legislation -kvalifioijat): 58 osumaa - mutta pudotti pois tunnetun hyvän osuman ("Pain in laboratory fishes", joka lähetettiin tämän päivän oikeassa koosteessa), koska paperi ei sisältänyt näitä tarkkoja sanoja.
- Laajennettu versio (lisättiin `pain`/`sentience`/`emotion`/`consciousness` samaan kvalifioijalistaan): 69 osumaa - palautti kalatutkimuksen mukaan, pudotusten pistokoe (25 nimikettä) vahvisti että jäljelle jäänyt kohina oli enää yksittäisiä rajatapauksia (esim. rutiininomaiset analgesia-protokollat jotka sisältävät sanan "pain"), ei enää massiivista määrää täysin aihepiirin ulkopuolista sisältöä.

Valittiin laajennettu (69 osuman) versio lopulliseksi - 57 % vähennys alkuperäisestä volyymista, katsausartikkeleiden osuus pysyi terveenä (16/69, 23 %), ja tunnettu hyvä osuma säilyi. Jäljelle jäävää pientä kohinaa ei yritetty poistaa täydellisesti kyselytasolla - se on tarkoituksella jätetty agentin vaiheen 2 harkinnan varaan, samalla periaatteella kuin koko projektin muissakin skilleissä (deterministinen haku voi olla epätäydellinen, agentin harkinta on se mikä lopulta ratkaisee).

**Todennus ennen käyttöönottoa, ei vain jälkikäteen.** Uusi kysely vietiin `fetch_pubmed.py`:hen vasta kun `esearch`-testi oli jo vahvistanut sekä osumamäärän että laadun - ja lopuksi ajettiin vielä itse `fetch_pubmed.py --days 30 --retmax 100` oikeasti varmistamaan että koko skripti (esearch → esummary → efetch) toimii uuden kyselyn kanssa alusta loppuun: 69 artikkelia, 16 katsausta/meta-analyysia, sama luku kuin suora `esearch`-testi ennusti.

## 7. Iteraatio 4: ajastuksen ensimmäinen oikea tarkistus paljasti sama vikatyyppi kuin `vegan-news-feed`:llä

Käyttäjä kysyi 2026-09-18 mitä tälle skillille on tapahtunut sen jälkeen kun ajastus otettiin käyttöön (iteraatio 2). `launchctl print` osoitti `runs = 2`, mutta historia sisälsi merkintöjä kolmelta eri päivältä (09-04, 09-06, 09-14) - ristiriita joka paljastui todennettaessa tarkemmin.

**Löydös: sunnuntai 2026-09-13 jäi kokonaan ajamatta, ja myöhässä oleva korvaava ajo (maanantai 09-14 klo 14:44, 44 min ajastetusta) tallensi kylläkin sisällön (5 artikkelia + 2 sisältöideaa) mutta EI run-merkintää.** `history.py --history-file ... gaps` vahvisti 09-13:n aidoksi aukoksi (ei yhtään merkintää), kun taas 09-14 ei näkynyt aukkona koska sillä oli muuta sisältöä - vain run-merkintä puuttui. Tämä on täsmälleen sama vikaluokka kuin `vegan-news-feed`:n iteraatio 6:ssa (ajastus jää välistä, myöhempi herääminen tuottaa "tämän päivän" ajon eikä oikeaa taannehtivaa täyttöä) - nyt nähty tässä skillissä ensimmäistä kertaa.

**Tärkeä ero päivittäiseen skilliin: tämä on todennäköisesti VÄHEMMÄN menetystä aiheuttava tässä skillissä.** `vegan-news-feed`:n 30h-ikkuna tarkoittaa että jokainen päivä on suurelta osin oma, erillinen sisältönsä - myöhässä oleva korvike menettää oikeasti sen päivän ainutlaatuisen sisällön. Tämä skilli sen sijaan hakee aina 7 päivän ikkunan taaksepäin - 09-14:n ajo kattoi jo suurelta osin saman ajanjakson minkä 09-13:n ajo olisi kattanut, koska ikkunat menevät päällekkäin. Ei vahvistettu tarkalleen mitä 09-13 olisi tuottanut, mutta ikkunoiden matemaattinen päällekkäisyys tekee todellisen sisältöhukan epätodennäköiseksi - toisin kuin päivittäisellä skillillä.

**Korjattu:** puuttuva run-merkintä (09-14, `"result": "sent", "item_count": 5`) lisättiin jälkikäteen suoraan `history.py record`:lla, merkittynä huomiolla mistä se johtui. Ei tehty erillistä "09-13"-taannehtivaa täyttöä, koska kyse ei arvion mukaan ole todellisesta sisältöaukosta - vain kirjanpitovirheestä.

**Sivuhavainto: `history.py gaps` ei ole viikkotahdin tietoinen.** Komento tarkistaa JOKAISEN päivän annetulta väliltä, riippumatta siitä ajetaanko skilliä päivittäin vai viikoittain - viikoittaiselle skillille tämä tuottaa paljon "aukkoja" jotka ovat itse asiassa täysin odotettuja väliviikon päiviä, ei oikeita ongelmia. Kuka tahansa (ihminen tai agentti) joka lukee `gaps`-komennon tulostetta tälle skillille tarvitsee tämän kontekstin, jotta ei tulkitse jokaista listattua päivää virheellisesti todelliseksi aukoksi.
