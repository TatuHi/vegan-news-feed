...
- **Hakusanojen kontribuutio ja päällekkäisyys** (valinnainen, ei tarvitse ajaa joka katselmoinnilla — verkkokutsuja vie hetken): `scripts/query_term_overlap.py` analysoi suoraan Google Newsia vasten, mitkä `feeds.md`:n Google News -hakujen yksittäiset OR-termit tuottavat tuloksia lainkaan (`"dead_term": true` = ei yhtään osumaa koko tarkastelujaksolla) ja kuinka paljon kunkin termin osumista on jonkin toisen termin (samalla kieli-/alue-asetuksella) jo löytämiä (`"redundancy_ratio"`). Käyttökelpoinen erityisesti jos "lähteiden kontribuutio" -havainto (yllä) osoittaa suomenkielisten lähteiden tuottavan vähän tai ei mitään — tämä työkalu erottaa "hakusana ei toimi lainkaan" ja "hakusana on redundantti toisen kanssa" toisistaan, konkreettisemmin kuin `history.py`:n data yksin pystyy. Ks. `../vegan-news-feed/PROCESS.md`:n iteraatiot 9-10 taustaksi miksi tämä rakennettiin, ja skriptin oma docstring käyttöohjeille. Yleiskäyttöinen — toimii myös muiden skillien (esim. `vegan-literature-feed`) Google News -tyylisten hakujen kanssa, ei vain `vegan-news-feed`:n.
...
- `scripts/query_term_overlap.py` — yleiskäyttöinen (ei sidottu mihinkään skilliin), stdlib-vain skripti joka analysoi Google News RSS -hakujen OR-termien kontribuution ja päällekkäisyyden suoraan Googlea vasten — ks. vaihe 3 "Hakusanojen kontribuutio ja päällekkäisyys" ja skriptin oma docstring
